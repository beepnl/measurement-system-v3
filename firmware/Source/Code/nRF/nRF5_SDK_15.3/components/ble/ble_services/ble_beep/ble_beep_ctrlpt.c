...
#include "app_util.h"
#define NRF_LOG_MODULE_NAME BEEP_CONTROL
#include "nrf_log.h"
...
uint32_t ble_sc_ctrlpt_init(ble_sc_ctrlpt_t * p_sc_ctrlpt, const ble_cs_ctrlpt_init_t * p_sc_ctrlpt_init)
{
    ...
    return characteristic_add(p_sc_ctrlpt->service_handle, &add_char_params, &p_sc_ctrlpt->sc_ctrlpt_handles);
}
 ...
 */
static uint32_t sc_ctrlpt_decode(uint8_t const * p_rcvd_val, uint8_t len, ble_sc_ctrlpt_val_t * p_write_val)
{
    int pos = 0;
...
static bool is_cccd_configured(ble_sc_ctrlpt_t * p_sc_ctrlpt)
{
    ...
    err_code = sd_ble_gatts_value_get(p_sc_ctrlpt->conn_handle, p_sc_ctrlpt->sc_ctrlpt_handles.cccd_handle, &gatts_value);
 ...
 */
static void on_ctrlpt_write(ble_sc_ctrlpt_t             * p_sc_ctrlpt, ble_gatts_evt_write_t const * p_evt_write)
{
	...
	NRF_LOG_INFO("BLE control point:");
    ...
    else
    {
        ...
        switch (rcvd_ctrlpt.opcode)
        {
            ...
            case BLE_SCPT_UPDATE_SENSOR_LOCATION:
                if (1)
                {
                    if (1) //is_location_supported(p_sc_ctrlpt, rcvd_ctrlpt.location))
                    {
                        ble_gatts_value_t gatts_value;
                        ...
                        {
                            err_code = sd_ble_gatts_value_set(p_sc_ctrlpt->conn_handle,  p_sc_ctrlpt->service_handle, &gatts_value);
                            if (err_code != NRF_SUCCESS)
                            {
                                // Report error to application
                                if (p_sc_ctrlpt->error_handler != NULL)
    ...
    p_sc_ctrlpt->response.len = ctrlpt_rsp_encode(p_sc_ctrlpt, &rsp, p_sc_ctrlpt->response.encoded_ctrl_rsp);
