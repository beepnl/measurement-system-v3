 ...
 */
static LoRaMacCommandsCtx_t macNvmCtx;
...
static MacCommand_t* MallocNewMacCommandSlot( void )
{
    ...
    while( IsSlotFree( ( const MacCommand_t* )&macNvmCtx.MacCommandSlots[itr] ) == false )
    {
        itr++;
    ...
    return &macNvmCtx.MacCommandSlots[itr];
}
...
/*
 * \brief Wrapper function for the macNvmCtx
 */
...
LoRaMacCommandStatus_t LoRaMacCommandsInit( LoRaMacCommandsNvmEvent commandsNvmCtxChanged )
{
    ...
    memset1( ( uint8_t* )&macNvmCtx, 0, sizeof( macNvmCtx ) );

    LinkedListInit( &macNvmCtx.MacCommandList );
...
LoRaMacCommandStatus_t LoRaMacCommandsRestoreNvmCtx( void* commandsNvmCtx )
{
    ...
    {
        memcpy1( ( uint8_t* )&macNvmCtx, ( uint8_t* )commandsNvmCtx, sizeof( macNvmCtx ) );
        return LORAMAC_COMMANDS_SUCCESS;
    }
    else
    {
...
{
    *commandsNvmCtxSize = sizeof( macNvmCtx );
    return &macNvmCtx;
}
...
LoRaMacCommandStatus_t LoRaMacCommandsAddCmd( uint8_t cid, uint8_t* payload, size_t payloadSize )
{
    ...
    if( LinkedListAdd( &macNvmCtx.MacCommandList, newCmd ) == false )
    {
    ...
    macNvmCtx.SerializedCmdsSize += ( CID_FIELD_SIZE + payloadSize );
...
LoRaMacCommandStatus_t LoRaMacCommandsRemoveCmd( MacCommand_t* macCmd )
{
    ...
    if( LinkedListRemove( &macNvmCtx.MacCommandList, macCmd ) == false )
    {
        return LORAMAC_COMMANDS_ERROR_CMD_NOT_FOUND;
    }

    macNvmCtx.SerializedCmdsSize -= ( CID_FIELD_SIZE + macCmd->PayloadSize );
...
LoRaMacCommandStatus_t LoRaMacCommandsGetCmd( uint8_t cid, MacCommand_t** macCmd )
{
    ...
    // Start at the head of the list
    curElement = macNvmCtx.MacCommandList.First;
...
LoRaMacCommandStatus_t LoRaMacCommandsGetSizeSerializedCmds( size_t* size )
{
    ...
    *size = macNvmCtx.SerializedCmdsSize;
    return LORAMAC_COMMANDS_SUCCESS;
}
