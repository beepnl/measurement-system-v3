...
LoRaMacCryptoStatus_t LoRaMacCryptoGetKey( KeyIdentifier_t keyID, uint8_t* key )
{
    if( SecureElementGetKey( keyID, key ) != SECURE_ELEMENT_SUCCESS )
    {
        return LORAMAC_CRYPTO_ERROR_SECURE_ELEMENT_FUNC;
    }
    if( ( keyID == GEN_APP_KEY ) || ( keyID == APP_KEY ) )
...
LoRaMacCryptoStatus_t LoRaMacCryptoPrepareJoinRequest( LoRaMacMessageJoinRequest_t* macMsg )
{
    ...
    CryptoCtx.NvmCtx->DevNonce = get_random_devNonce();
#else
    CryptoCtx.NvmCtx->DevNonce++;
#endif
    NRF_LOG_INFO("Join DevNonce: %u/0x%04X", CryptoCtx.NvmCtx->DevNonce, CryptoCtx.NvmCtx->DevNonce);
    CryptoCtx.EventCryptoNvmCtxChanged( );
