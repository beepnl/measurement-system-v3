...
SecureElementStatus_t SecureElementGetKey( KeyIdentifier_t keyID, uint8_t* key )
{
    for( uint8_t i = 0; i < NUM_OF_KEYS; i++ )
    {
        if( SeNvmCtx.KeyList[i].KeyID == keyID )
        {
            ...
            {  // Decrypt the key if its a Mckey
				...
				// Copy the key first and decrypt in the given storage array
                memcpy1( decryptedKey, SeNvmCtx.KeyList[i].KeyValue, KEY_SIZE );

                retval = SecureElementAesEncrypt( decryptedKey, 16, MC_KE_KEY, key );
                return retval;
            }
            else
            {
				// Copy the key
                memcpy1( key, SeNvmCtx.KeyList[i].KeyValue, KEY_SIZE );
                return SECURE_ELEMENT_SUCCESS;
            }
        }
    }
