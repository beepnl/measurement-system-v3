...
NvmCtxMgmtStatus_t NvmCtxMgmtRestore( void )
{
	...
	NRF_LOG_INFO("CryptoNvmCtxDataBlock restore = size: %u, status: %s", mibReq.Param.Contexts->CryptoNvmCtxSize, (status != NVMM_SUCCESS) ? "Fail" : "Succes" );
    ...
    NRF_LOG_INFO("SecureElementNvmCtxDataBlock restore = size: %u, status: %s", mibReq.Param.Contexts->SecureElementNvmCtxSize, (status != NVMM_SUCCESS) ? "Fail" : "Succes" );
    ...
    NRF_LOG_INFO("FCntHandlerNvmCtxDataBlock restore = size: %u, status: %s", mibReq.Param.Contexts->FCntHandlerNvmCtxSize, (status != NVMM_SUCCESS) ? "Fail" : "Succes" );
